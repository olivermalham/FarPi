...
def draw_overlay(frame, colour, overlay_text, origin=None, font_scale=1.2):
...
def draw_crosshair(frame, colour, box=0, ticks=True):
    width = len(frame)
    height = len(frame[0])
    ...
    if box > 0:
        cv2.rectangle(frame, (height_cen - half_box, width_cen - half_box), (height_cen + half_box, width_cen + half_box), colour, 1)
...
def gen_depth_frames():
    ...
    while True:
        ...
        distance = get_range(depth_frame, 40)
        ...
        depth_color_mapped = draw_overlay(depth_color_mapped, (0, 0, 0), f"Frame No. {frame_count}")
        depth_color_mapped = draw_overlay(depth_color_mapped, (0, 0, 0), f"{distance:.2f}m", font_scale=1.0, origin=(int(depth_frame.get_width()/2) + 25, int(depth_frame.get_height()/2) + 15))
        depth_color_mapped = draw_crosshair(depth_color_mapped, (0, 0, 0), box=40, ticks=False)
...
def gen_ir_frames():
    ...
    while True:
        ...
        ir_image = draw_overlay(ir_image, (255, 255, 255), f"Frame No. {frame_count}")
